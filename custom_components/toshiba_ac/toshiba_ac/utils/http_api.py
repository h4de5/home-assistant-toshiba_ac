...
from ..utils import retry_with_timeout, retry_on_exception
...
class ToshibaAcHttpApiRejectError(ToshibaAcHttpApiError):
    pass
            ...
            if response.status == 200:
                ...
                if json["IsSuccess"]:
                    ...
                    if json["StatusCode"] == "InvalidUserNameorPassword":
                        logger.error("Toshiba authentication error")
                        raise ToshibaAcHttpApiAuthError(json["Message"])

                    raise ToshibaAcHttpApiError(json["Message"])
            # Don't retry if unauthorized access
            elif response.status == 403:
                logger.error("Toshiba unauthorized or SAS token expiration error")
                raise ToshibaAcHttpApiRejectError(await response.text())
